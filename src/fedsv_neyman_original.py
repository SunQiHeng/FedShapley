...
from src.util import get_dataset, average_weights, exp_details, SVAtt_weights,avgSV_weights,avgSVAtt_weights
...
def solver():
    ...
    sampling_variance = np.ones(len(index))
    for epoch in tqdm(range(args.epochs)):
        ...
        for i in range(len(local_weights)):
            ...
            shapley[i] = current_acc/len(index)
        ...
        for i in range(2,len(sampling_variance)):
            sampling_number[i] = sampling_variance[i]/(sampling_variance.sum()) * sampling_tot
