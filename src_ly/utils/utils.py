...
from src.utils.sampling import mnist_iid, mnist_noniid, mnist_noniid_unequal, cifar_iid, cifar_noniid, FashionMnist_noniid
...
def attAvg_weights(w, clients_acc):
    """
    Returns the average of the weights.
    """
    s = 0
    for i in range(len(clients_acc)):
        clients_acc[i] = 1 - clients_acc[i]
        s += clients_acc[i]
    w_avg = copy.deepcopy(w[0])
    for key in w_avg.keys():
        w_avg[key] = w_avg[key] * clients_acc[0] / s
        for i in range(1, len(w)):
            w_avg[key] += w[i][key] * clients_acc[i] / s
    return w_avg


def accAvg_weights(w, clients_acc):
    """
    Returns the average of the weights.
    """
    s = 0
